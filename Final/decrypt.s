...
getMsg:	
	mov	r9, #8			/* Value to left shift by */
	/* Get Significant bits of Length */
	/* Parameter: r0 is set to the memory */
	/* Parameter: r1 is set to the current itterator value */
	mov	r0, r4			/* Image Memory Buffer */
	mov	r1, r5			/* Itterator Image File */
	bl	readByte		
	mov	r6, r0			/* Get Least Significant Bits of Message Length */
	mov	r5, r1			/* Store Itterator */ 
	
	mul	r9, r9, r8		/* Get Left shift value */
	mov	r6, r6, lsl r9		/* Left shift by 0, 8, 16 */
	orr	r7, r7, r6		/* orr the values */

	add	r8, r8, #1		/* Add 1 each time */
	cmp	r8, #4			/* Compare with 3 */
	blt	getMsg

	ldr 	r2, =msgLength
	str	r7, [r2]		/* Store Message Length */		

	mov	r6, r7			/* Store Message Length back to r6 */
...
secretReq:	.asciz	"Enter name of secret image"
