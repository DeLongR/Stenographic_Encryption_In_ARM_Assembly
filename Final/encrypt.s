...
debug6:
	...
	ldr	r2, =msgLength
	ldr	r2, [r2]		/* r2 is set to value to store */
	mov	r5, r2			/* Store to r5 */
msgStore:
	cmp	r9, #0			/* If itterator is first loop don't shift */	
	movne	r5, r5, lsr #8 	

	and	r2, r5, r4		/* And 255 to get only the relevant bits */
	...
	add	r9, r9, #1		/* Add 1 */
	/* if greater than greater than quit */
	cmp	r9, #4
	bne	msgStore
...
/*************************** Constants ***** **********************/
/* Image File Request */
imageReq:	.asciz 	"Enter Name of Image to Encrypt"

/* Text File Request */
textReq:	.asciz 	"Enter Name of Secret Message to Encrypt"
...
secretOutput:		.asciz	"Secret message stored into %s\n"
